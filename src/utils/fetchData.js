...
export const getAllFeeds = async (firestoreDb) => {
  const feeds = await getDocs(
    query(collection(firestoreDb, "accident"))
...
export const getSpecificVideo = async (firestoreDb, from) => {
  // const videoRef = doc(firestoreDb, "accident", from);
console.log(from);
  const videoSnap = await getDocs(
    query(
      collection(firestoreDb, "accident"),
      where("from", "==", from)
    )
  );

  return videoSnap.docs.map((doc) => doc.data());
};
